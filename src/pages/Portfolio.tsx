        ...
        <Card className="card-gradient">
          ...
          <CardContent>
            <div className="space-y-4">
              {[
                { sector: "Financial Services", percentage: 48.5, value: 31250, color: "bg-primary" },
                { sector: "Telecommunications", percentage: 32.1, value: 12850, color: "bg-accent" },
                { sector: "Banking", percentage: 19.4, value: 18500, color: "bg-secondary" }
              ].map((item) => (
          ...
          <CardContent>
            <div className="grid grid-cols-2 gap-4 mb-6">
              <div className="text-center p-4 rounded-lg bg-muted/20">
                ...
                <div className="text-xl font-bold text-bull">KES 2,450</div>
                <div className="text-xs text-muted-foreground">+12.5% vs last year</div>
              </div>
              <div className="text-center p-4 rounded-lg bg-muted/20">
                <div className="text-sm text-muted-foreground mb-1">Dividend Yield</div>
                <div className="text-xl font-bold text-primary">3.9%</div>
                <div className="text-xs text-muted-foreground">Above market avg</div>
              </div>
            </div>
            
            <div className="space-y-3">
              <h4 className="font-medium text-sm">Recent Dividends</h4>
              {[
                { stock: "EQTY", amount: 1250, date: "Dec 2024", yield: 4.0 },
                { stock: "SAFCOM", amount: 850, date: "Nov 2024", yield: 6.6 },
                { stock: "SCBK", amount: 350, date: "Oct 2024", yield: 1.9 }
              ].map((dividend) => (
                <div key={`${dividend.stock}-${dividend.date}`} className="flex items-center justify-between p-3 rounded-lg bg-muted/20">
                  <div>
                    <div className="font-medium">{dividend.stock}</div>
                    <div className="text-xs text-muted-foreground">{dividend.date}</div>
                  </div>
                  <div className="text-right">
                    <div className="font-medium text-bull">KES {dividend.amount}</div>
                    <div className="text-xs text-muted-foreground">{dividend.yield}% yield</div>
                  </div>
                </div>
              ))}
            </div>
